...
function exp_basis_integral(t, i, a, num_params, time_period)
    if i == 1
        ...
        if isapprox(a, 0.0, atol=1e-15) # Handle a=0 case to avoid division by zero
            return t
        else
    ...
    elseif i <= (num_params+1)/2
        ...
        return exp_sin(t, a, freq)
    else
        # Similarly for exp_cos
        freq = 2 * pi * (i - ((num_params + 1)/ 2)) / (time_period)
...
function control_constraints(model, control_params, uncert, num_params, control_bounds, time_samples, time_period, terminal_time)

    num_grids = 150
    grid = LinRange(0.0, terminal_time, num_grids)
    ...
    for i in 1:length(grid)
        for j in 1:size(time_samples)[1]
            @constraint(model, lower_bound <= control(grid[i], control_params + uncert[j, :], num_params, time_period) <= upper_bound)
        end
    end
    ...
    for i in 1:size(time_samples)[1]
        ...
        @constraint(model, lower_bound <= control(terminal_time, control_params + uncert[i, :], num_params, time_period) <= upper_bound)
    end

    @constraint(model, control(0, control_params, num_params, time_period) == 0.0)
    


end

function slack_constraint(model, terminal_state, lambda, gamma, control_params, slack, states_sampled, uncert, num_samples)

    Q = deserialize("Q_matrix.dat")  # Load the Q matrix from a file

    for i in 1:num_samples

        states_sampled_error = states_sampled[:, i] - terminal_state
        @constraint(model,  10 * ( 0.5 * lambda * control_params' * Q * control_params + 0.5 * sum(states_sampled_error.^2) * ( 1 / (length(states_sampled_error) + 1) ) - 0.5 * gamma * uncert[i, :]' * Q * uncert[i, :] ) <=  slack)
    end
end
...
function dynamics_constraints(model, initial_state, state_matrix, control_matrix, states_sampled, control_params, uncert, num_samples, num_params, time_period, terminal_time)
    ...
    state_eigen_values = eigen(A).values
    state_eigen_vectors = eigen(A).vectors
