...
function state_params()
    ...
    theta_n = diagm([theta(x/discrete_points) for x in 1:discrete_points])
    sigma_n = diagm([sigma(x/discrete_points) for x in 1:discrete_points])
    alpha_n = diagm([alpha(x/discrete_points) for x in 1:discrete_points])
    ...
    q_0 = -neumann_0 / (dirichlet_0  - h * neumann_0)
