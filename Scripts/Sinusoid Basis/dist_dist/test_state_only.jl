...
initial_state = [100 * ((x/N)^2) * (1 - x/N)^3 * sin(initial_modes * pi * x/N) for x in 1:N]  
terminal_state = [4 * ((x/N)) * (1 - x/N) for x in 1:N]
...
for i in 1:plot_samples
    trajectory[i, :] = dynamics_constraints(initial_state, state_matrix, control_matrix, control_1, uncert_1, i * terminal_time / plot_samples, num_params)
end
