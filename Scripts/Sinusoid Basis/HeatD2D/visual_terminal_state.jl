...
control_1 = deserialize("Recent_Control_Trajectory_$N.dat")
...
terminal_controlled_state = zeros(N)



for i in 1:num_lines   
    
    terminal_state_1 = dynamics_constraints(initial_state, state_matrix, control_matrix, control_1, opt_uncert[i, :], terminal_time, num_params, time_period) 

    for j in 1:N
        terminal_controlled_state[j] = terminal_state_1[j]
    end

    lines!(ax, x_space, terminal_controlled_state,
        color = cmap[i],
        linewidth = 3.5,
        label = "Input_$i"
    )
# lines!(ax, dynamics_constraints(initial_state, state_matrix, control_matrix, control_1, opt_uncert[i, :], terminal_time, num_params, time_period); label = "Actual Terminal State")
...
save("HeatTerminalDist_$N.png", fig, px_per_unit = 2)

fig

println("Plot saved for N = $N")
