...
time_period = 100.0                                                                                 # Time period for the control
...
disturbance_upper_bound = 0.0
disturbance_lower_bound = -0.0
time_lower_bound = 0.0
time_upper_bound = terminal_time

single_psuedo_uncert_row_lower_bound = vcat([0.0], fill(-1.0, num_params))
single_psuedo_uncert_row_upper_bound = vcat([0.0], fill(1.0, num_params))
...
function uncert_converter(uncert_array)
      ...
      for i in 1:num_samples
         denominator = sum(abs.(psuedo_uncert[2:end, i]))  # Compute the denominator for each sample
         uncert[i, :] = uncert_coeff[i] * (psuedo_uncert[2:end, i] ./ denominator)  # Normalize the uncertainty coefficients
...
for i in 1:de_iterations
   ...
   control_commands = control_trajectory(state_matrix, control_matrix, control_bounds, num_samples, num_params, terminal_time, opt_uncert, state_dim, initial_state, terminal_state, gamma, lambda, opt_time_samples)
