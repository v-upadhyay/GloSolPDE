...
############################################################################################
   ...
   later for intermediate constraints.
	...
	1. main.jl containing the differential evolution driven maximization over 
      the uncertainty set.
...
terminal_state = [0.0*(x/N) for x in 1:N]                                                                            # Initial distribution for the PDE
...
control_bounds = [-0.07, 0.07]                                                   # Control bounds
...
disturbance_upper_bound = 0.005
disturbance_lower_bound = -0.005
...
sip_lower_bound = vcat([disturbance_lower_bound], flat_psuedo_uncert_lower_bounds, time_samples_lower_bounds)
sip_upper_bound = vcat([disturbance_upper_bound], flat_psuedo_uncert_upper_bounds, time_samples_upper_bounds)
...
function maximization_cost(uncert_samples)
   ...
   #  uncert = uncert_projector(uncert_matrix, uncert_radius, uncert_0, num_samples, num_params)  # Project the uncertainty samples onto the hyperplane 
    uncert = uncert_matrix  # Use the uncertainty samples directly without projection
