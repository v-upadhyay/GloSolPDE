...
control_array = [control(ti, control_params_array, num_params, time_period) for ti in t]


for i in 1:plot_samples
    trajectory[i, :] = vcat(dynamics_constraints(initial_state, state_matrix, control_matrix, control_1, uncert_1, i * terminal_time / plot_samples, num_params, time_period), [control_array[i]])
end
